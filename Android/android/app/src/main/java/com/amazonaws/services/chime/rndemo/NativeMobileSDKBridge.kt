...
class NativeMobileSDKBridge(
    ...
    private val webRtcPermissionPermission =
            arrayOf(
    ...
    fun startMeeting(meetingInfo: ReadableMap, attendeeInfo: ReadableMap) {
        ...
        currentActivity?.let { activity ->
            ...
            try {
                ...
                val meetingSession =
                        sessionConfig?.let {
                            DefaultMeetingSession(it, logger, activity.applicationContext)
                        }
    ...
    fun bindVideoView(viewIdentifier: Double, tileId: Int) {
        logger.info(
                TAG,
                "Called bindVideoView for tileId: $tileId with identifier: $viewIdentifier"
        )
    }
    ...
    fun unbindVideoView(tileId: Int) {
        ...
        meetingSession?.run { audioVideo.unbindVideoView(tileId) }
    }
            ...
            WEBRTC_PERMISSION_REQUEST_CODE -> {
                val isMissingPermission: Boolean =
                        grantResults?.isEmpty()
                                ?: false ||
                                grantResults?.any { PackageManager.PERMISSION_GRANTED != it }
                                        ?: false
