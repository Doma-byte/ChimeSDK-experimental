...
const Message = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    if (Platform.OS === "android") {
      soundRef.current = new Sound(ringtone, Sound.MAIN_BUNDLE, (error) => {
        if (error) {
  ...
  const onDisplayNotification1 = async (details) => {
    try {
  ...
  useEffect(() => {
    ...
    socket.current.on("playRingtone", (details) => {
      ...
      if (details.recipientId === value1.id) {
        if (Platform.OS === "android") {
          ...
          onDisplayNotification1(details);
    ...
    return () => {
      ...
      socket.current.off("callAccepted", handleCallAcceptance);
    };
  }, [socket]);
